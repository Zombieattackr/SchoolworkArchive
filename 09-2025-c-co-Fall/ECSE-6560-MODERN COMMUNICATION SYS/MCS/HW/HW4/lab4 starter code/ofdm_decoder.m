function [op_struct, params] = ofdm_decoder(rx_vec_air, params)
% OFDM Decoder

assert(isempty(rx_vec_air)<1, 'Rx data is empty length 0')
raw_rx_dec = rx_vec_air;

%% Detect packet
[lts_ind,params] = rx_pack_detect(raw_rx_dec,params);
...
if(lts_ind<0||isempty(pack_ind_list))
    error('LTS correlation negative or no packet can be extracted')
end
...
for pidx = 1:length(pack_ind_list)
    ...
    try
        ...
        rx_pkt_and_preamble_range = lts_ind_loop:(lts_ind_loop+2.5*params.N_SC+tx_num_origsamp-1);
        raw_rx_dec_iter = raw_rx_dec(rx_pkt_and_preamble_range);
        
        %% CFO Correction 
        [rx_vec_cfo_corr,params] = rx_cfo_correction(raw_rx_dec_iter,1,params);
        
        %% Channel Estimation
        [rx_H_est,params] = rx_channel_estimation(rx_vec_cfo_corr,1,params);
    ...
    if(params.DO_DECODE)
        %% Rx Payload
        syms_eq_mat = rx_equalize_signal(rx_vec_cfo_corr,1,rx_H_est,params);
        %% SFO and phase correction
        [syms_eq_pc_mat,params] = rx_sfo_and_phase_error_correction(syms_eq_mat,params);
 
        payload_syms_mat = syms_eq_pc_mat(params.SC_IND_DATA, :);
        op_struct.rx_syms(:,pidx) = reshape(payload_syms_mat, params.N_DATA_SYMS,1);
        %% Demodulate 
        op_struct=rx_demodulation(pidx,op_struct,params);
        
        % Calculate Stats
        op_struct.evm_mat(:,:,pidx) = abs(payload_syms_mat - params.tx_syms_mat).^2;
        op_struct.aevms(pidx)= mean(op_struct.evm_mat(:,:,pidx),'all');
        op_struct.snr(pidx) = 10*log10(1./op_struct.aevms(pidx));
        [op_struct.bernum(pidx),op_struct.berratio(pidx)] = biterr(params.tx_data,op_struct.rx_data(:,pidx));
    end
end
% Plotting
if(params.plot_flag)
    for pidx = 1:min(10,params.num_packets)
        ...
        sgtitle(sprintf('Channel Estimate, BER=%d',mean(op_struct.berratio)))
        if(params.DO_DECODE)
