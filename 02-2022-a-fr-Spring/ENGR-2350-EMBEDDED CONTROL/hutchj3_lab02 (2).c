...
// Does RGB example stuff with BMPs and ALSO returns integer color value
int RGB_Input() {
    uint8_t pressed = 8;
    uint8_t held = 0;
...
// Does RGB example stuff with BMPs, no value returned
void RGB_Example() {
    uint8_t held = 0;
    bp0 = !GPIO_getInputPinValue(GPIO_PORT_P4, GPIO_PIN0);
    bp1 = !GPIO_getInputPinValue(GPIO_PORT_P4, GPIO_PIN2);
    ...
    bp4 = !GPIO_getInputPinValue(GPIO_PORT_P4, GPIO_PIN6);
    bp5 = !GPIO_getInputPinValue(GPIO_PORT_P4, GPIO_PIN7);
    if(bp0||bp1||bp2||bp3||bp4||bp5){
        held=1;}
    while(held) { //sets bp0-5 as bumper values, checks if any are true and sets that as pressed value,
                                //sets LED to that color and waits for release
        if(bp0) {
            // RED
            __delay_cycles(240e3);
...
void Timer_ISR(){ // General ISR function which will be automatically called by hardware upon overflow
    ...
    if(green){
        if(greentoggle==5){
            ...
            greentoggle = 0;
        } else if(greentoggle < 5){
            // Add 1 to greentoggle, indicating another 5ms passed
            greentoggle += 1;
            // Turn OFF
    ...
    if(red){
        if(redtoggle==5){
            ...
            // Add 1 to redtoggle, indicating another 5ms passed
            redtoggle += 1;
            // Turn OFF
            GPIO_setOutputLowOnPin(GPIO_PORT_P3, GPIO_PIN3|GPIO_PIN2);
        }
